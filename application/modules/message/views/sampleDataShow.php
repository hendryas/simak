		...
		<div class='innerBox'>
			<div class='user px-3 py-2'>
				<div id='avtar_and_details' class=''>
					<div id='user_avtar' style="background-image: url('<?= base_url('assets/images/') . $image ?>'); background-size: 100% 100%;">
						<div id='online'></div>
						<input type='hidden' name='hdn' id='hidden_id' value="<?php echo $data[$i]['id_user']; ?>">
					</div>
					<div id='user_details' class='px-2'>
						...
						<p class='m-0' id="title">
							...
							for ($j = 0; $j < count($last_msg); $j++) {
								if ($data[$i]['id_user'] == $last_msg[$j]['sender_id']) {

									$output = ($last_msg[$j]['message']);
								} elseif ($data[$i]['id_user'] == $last_msg[$j]['receiver_id']) {
				...
				<div>
					<p id="time">
						...
						$messageTime = "";
						for ($j = 0; $j < count($last_msg); $j++) {
							if ($data[$i]['id_user'] == $last_msg[$j]['sender_id'] || $data[$i]['id_user'] == $last_msg[$j]['receiver_id']) {
