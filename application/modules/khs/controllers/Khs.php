...
class Khs extends CI_Controller
{
  ...
  {
    $data['title'] = 'Program Studi Management (KHS)';
    ...
    if ($idRole == 4) {
      $data['dataKhs'] = $this->Khs_model->getDataById($idUser)->result_array();
    } else {
    ...
    if ($this->form_validation->run() == false) {
      ...
      $ext = end($dname);
      $new_file = $_FILES['file_upload']['name'] = strtolower('khs' . '_' . $semester . '_' . $nama_user . '_' . $thbl . '-' . $his . '.' . $ext);

      if ($file_upload != null) {
        $file_name1 = 'khs' . '_' . $semester . '_' . $nama_user . '_' . $thbl . '-' . $his;
        ...
        if ($this->upload->do_upload('file_upload')) {
          ...
          $this->Khs_model->insertData($data);
  ...
  public function editData()
  {
    ...
    if ($file_upload != null) {
      ...
      if ($this->upload->do_upload('file_upload')) {
        ...
        $this->Khs_model->updateData($id_khs, $data);
        $this->session->set_flashdata('message', '<div class="alert alert-success text-center" role="alert">
  ...
  public function deleteData($id)
  {
    ...
    $this->Khs_model->updateData($id, $data);
