$(document).ready(function () {
		...
		return new Promise(function (resolve, reject) {
			...
			$.get("Message/ownerDetails", function (data) {
				...
				if (
					...
					owenerProfileBio.innerHTML =
						bio.length > 28 ? bio.slice(0, 28) + "..." : bio;
				} else {
	...
	function setUserDetails(data) {
		...
		var avtar = `./../../../assets/images/profile_1.png`;
		...
		data[0]["bio"].length > 1
			? $("#details_of_bio").html(data[0]["bio"])
		...
		data[0]["phone"].length > 1
			? $("#details_of_mobile").html(
					`<span><i class="fas fa-mobile-alt text-dark pr-2"></i></span>${data[0]["phone"]}`
			  )
			: $("#details_of_mobile").html(
		...
		data[0]["address"].length > 1
			? $("#details_of_location").html(
					`<span><i class="fas fa-map-marker-alt text-dark pr-2"></i></span>${data[0]["address"]}`
			  )
			: $("#details_of_location").html(
